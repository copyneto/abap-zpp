...
define view entity ZI_PP_CRIAR_INVENTARIO_ITEM
  as select from ztpp_ajust_inv_i
  association        to parent ZI_PP_CRIAR_INVENTARIO_HEADER as _Header          on  $projection.documentoUuid = _Header.documentoUuid
  ...
  association [0..*] to ZI_PP_AJUSTE_INVENTARIO_MSG          as _Messages        on  $projection.documentoItemUuid = _Messages.DocumentoItemUUID
                                                                                 and $projection.documentoUuid     = _Messages.DocumentoUUID
  association [0..1] to ZI_PP_AJUSTE_INVENTARIO_STATUS       as _ItemStatus      on  $projection.Status = _ItemStatus.StatusId
{
  key documentoitemuuid                                                                                                     as documentoItemUuid,
      ...
      @Semantics.quantity.unitOfMeasure: 'Unit'
      case counting when 0
      then 0
      else
      cast( quantity as abap.dec( 13, 3 )) - cast( counting as abap.dec( 13, 3 ) )
      end                                                                                                                   as Balance,
      @Semantics.amount.currencyCode: 'Currency'
      ( cast( quantity as abap.dec( 13, 3 )) - cast( counting as abap.dec( 13, 3 ) ) ) * cast( price as abap.dec( 13, 3 ) ) as TotalBalance,
      cast( '' as abap.char(10))                                                                                            as Accuracy,
